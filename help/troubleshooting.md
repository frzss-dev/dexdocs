...
>
> the transaction cannot succeed due to error: execution reverted: FRZDEXrouter: insufficient\_output\_amount.
...
However, there's also the chance that you're trying to trade a scam token which cannot be sold. In this case, FRZDEX isn't able to block a token or return funds.
{% endtab %}
{% endtabs %}
...
There currently isn't a simple way to solve this issue from the FRZDEX website: you'll need to interact with the contract directly. You can add liquidity directly via the Router contract, while setting amountAMin to a small amount, then withdrawing all liquidity.
...
![](<../.gitbook/assets/FRZDEX-k-solution (2).png>)
...
{% tab title="Reason" %}
**This issue (while swapping) is caused by tokens which have hard-coded the V1 FRZDEX router into their contract.**
...
You don't have enough SYRUP in your wallet to unstake from the FRZW-FRZW pool.

**Get at least as much SYRUP as the amount of FRZW that you’re trying to unstake.**

1. Buy SYRUP on the exchange. If you want to unstake 100 FRZW, you need at least 100 SYRUP.
...
This will unstake your staked tokens and lose any uncollected FRZW yield.

{% hint style="warning" %}
**This will lose any yield that you haven’t harvested yet.**
{% endhint %}

To stop this happening again, **don’t sell your SYRUP.** You still need it to unstake from the “Stake FRZW Earn FRZW” pool.

This error has happened because you have sold or transferred SYRUP tokens. SYRUP is minted in a 1:1 ratio to FRZW when you stake in the FRZW-FRZW Syrup Pool. SYRUP must be burned at a 1:1 ratio to FRZW when calling leaveStaking (unstaking your FRZW from the pool), so if you don't have enough, you can't unstake from the pool.
...
{% tabs %}
{% tab title="Solution" %}
Install the official browser extension to connect, or read our guide on [how to connect a wallet to FRZDEX](https://docs.FRZDEX.finance/get-started/connection-guide).
{% endtab %}
{% endtabs %}
...
1. Go to “Collectible” page, then come back to profile page.\
   If you can’t find the link, go to [https://FRZDEX.finance/collectibles](https://FRZDEX.finance/collectibles) directly.
...
\
It may make a conflict between wallets. This is out of FRZDEX's control and we can do nothing.
